...
class NewsDetailViewModelTest{
    ...
    private val dummyDetilNews = DataDummy.generateDummyNewsEntity()[0]

    @Before
    fun setUp(){
        newsDetailViewModel = NewsDetailViewModel(newsRepository)
        newsDetailViewModel.setNewsData(dummyDetilNews)
    }
    ...
    fun `when bookmarkStatus false Should call saveNews` () = runTest{
        ...
        `when`(newsRepository.isNewsBookmarked(dummyDetilNews.title)).thenReturn(expectedBoolean)
        newsDetailViewModel.apply {
            bookmarkStatus.getOrAwaitValue()
            changeBookmark(dummyDetilNews)
        }
        Mockito.verify(newsRepository).saveNews(dummyDetilNews)
    }
    ...
    fun `when bookmarkStatus true Should call deleteNews`() = runTest {
        ...
        newsDetailViewModel.changeBookmark(dummyDetilNews)
        Mockito.verify(newsRepository).deleteNews(dummyDetilNews.title)
    }
